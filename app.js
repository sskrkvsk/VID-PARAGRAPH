...
const bruch = document.querySelector("body");

amount.focus();
amount.select();

form.addEventListener("submit", function (e) {
  // preventing only for the chalange. without this it'll be sending stuff to a server
  e.preventDefault();
  // the value that we get from our input is always a string. To make it Number we need to use parseInt function
  const value = parseInt(amount.value);
  const random = Math.floor(Math.random() * text.length);
  console.log(random);

  // empty. isNaN - is Not a Number = true or false.
  // less then 0
  // more then 9
  if (isNaN(value) || value < 0 || value > 9) {
    result.innerHTML = `<p class="result bruh">${text[random]}</p>`;

    bruch.classList.add("yo");
    audio.play();
  } else if (value === 0) {
    result.innerHTML = "";
  } else {
    bruch.classList.remove("yo");
    audio.pause();
    audio.currentTime = 0;
